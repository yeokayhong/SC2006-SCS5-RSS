...
class LtaApi:
    ...
    def get_train_service_alerts(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return data['value']  # Assuming the response contains a 'value' key with the alerts
            else:
                raise Exception(f"Failed to fetch train service alerts. Status code: {response.status_code}")
     ...
     def get_platform_crowd_density_realtime(self, train_line):
        ...
        try:
            ...
            raise Exception(f"Failed to fetch platform crowd density real-time: {e}")
    ...
    def get_platform_crowd_density_forecast(self, train_line):
        ...
        try:
            ...
            raise Exception(f"Failed to fetch platform crowd density forecast: {e}")
